...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/chat/network_analysis` | POST | Community detection, centrality, clustering for an entity |
| `/chat/entity_360` | POST | Full 360-degree view of an entity (properties, edges, provenance) |
| `/chat/entity_gaps` | POST | Missing information for an entity |
| `/chat/dossier` | POST | Generate a comprehensive dossier on an entity |
| `/chat/topic_map` | POST | Topic clustering across the graph |
| `/chat/graph_stats` | POST | Overall graph statistics and health |
| `/chat/contradictions` | POST | Find contradictions involving an entity |
| `/chat/situation_at` | POST | Reconstruct state of knowledge at a specific point in time |
| `/chat/fact_provenance` | POST | Full provenance chain for a fact |

---

### Multi-Agent Debate

Launch and manage structured multi-agent debates with 7 analysis modes.

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/debate/start` | POST | Start a new debate session |
| `/debate/{id}` | GET | Get debate status and results |
| `/debate/{id}` | DELETE | Delete a debate session |
| `/debate/{id}/run` | POST | Run the next round of a debate |
| `/debate/{id}/stop` | POST | Stop a running debate |
| `/debate/{id}/synthesize` | POST | Generate final synthesis |
| `/debate/{id}/stream` | GET | SSE stream for live debate progress |
| `/debate/{id}/agents` | PATCH | Edit agent configuration |
| `/debate/{id}/inject` | POST | Inject evidence into a running debate |

**Example -- start a Red Team debate:**

```bash
curl -X POST http://localhost:3030/debate/start \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "topic": "Russia will escalate military operations in 2026",
    "mode": "red_team",
    "rounds": 3
  }'
```

**Debate modes:** `analyze`, `red_team`, `devils_advocate`, `scenario_planning`, `delphi`, `sat`, `war_game`

---

### Documents & Facts

Manage documents, extracted facts, and provenance chains.

#### Documents

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/documents` | POST | List documents (with filters) |
| `/documents/content` | POST | Get document content |
| `/documents/passage` | POST | Get a specific passage from a document |
| `/documents/{label}` | DELETE | Delete a document |
| `/provenance` | POST | Get full provenance chain for an entity |
| `/ingest/reprocess-docs` | POST | Reprocess all documents |
| `/ingest/reprocess-doc/{label}` | POST | Reprocess a single document |

#### Facts

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/facts` | POST | List extracted facts (with filters) |
| `/facts/{label}/confirm` | POST | Confirm a pending fact |
| `/facts/{label}/debunk` | POST | Mark a fact as debunked |

---

### Conflicts & Contradictions

Detect and resolve conflicting knowledge.

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/conflicts` | GET | List detected contradictions |
| `/conflicts/resolve` | POST | Resolve a conflict (supersede, contest, dispute) |

**Example:**

```bash
curl http://localhost:3030/conflicts \
  -H "Authorization: Bearer $TOKEN"
```
...
Manage server configuration, onboarding, models, and domain taxonomy.
...
| `/config` | POST | Update configuration (embedder, LLM, NER, system prompt) |
| `/config/status` | GET | Configuration readiness and wizard state |
| `/config/wizard-complete` | POST | Mark onboarding wizard as complete |

#### Model Management

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/config/onnx-model` | GET | Check ONNX embedding model status |
| `/config/onnx-model` | POST | Upload ONNX embedding model |
| `/config/onnx-download` | POST | Download ONNX embedding model by name |
| `/config/gliner2-download` | POST | Download GLiNER2 NER model |
| `/config/ner-download` | POST | Download NER model |
| `/config/rel-download` | POST | Download relation extraction model |
| `/config/ner-model` | GET | Check NER model status |
| `/config/rel-model` | GET | Check relation model status |
| `/config/ollama-pull` | POST | Pull an Ollama model |

#### Relation Templates

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/config/relation-templates/export` | GET | Export relation templates |
| `/config/relation-templates/import` | POST | Import relation templates |
| `/config/relation-types` | GET | List all known relation types |

#### Domain Taxonomy

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/config/domains` | GET | List configured domains |
| `/config/domains` | POST | Set domains |
| `/config/domains/suggest` | GET | LLM-suggested domains from graph content |
| `/config/domains/classify` | POST | Auto-classify entities into domains |

#### Knowledge Base Sources

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/config/kb` | GET | List KB endpoints (Wikidata, DBpedia) |
| `/config/kb` | POST | Add a KB endpoint |
| `/config/kb/{name}` | DELETE | Remove a KB endpoint |
| `/config/kb/{name}/test` | POST | Test a KB endpoint |

#### Entity Labels & Node Types

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/config/node-types` | GET | List all node types in the graph |
| `/config/entity-labels` | GET | Get entity label configuration |
| `/config/entity-labels` | POST | Set entity label configuration |
...
| `/auth/setup` | POST | Initial admin setup (first run) |
...
| `/auth/users/{username}` | PUT | Update user (admin only) |
| `/auth/users/{username}` | DELETE | Delete user (admin only) |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/admin/reset` | POST | Reset the database (destructive) |
| `/admin/dedup-edges` | POST | Deduplicate edges in the graph |
