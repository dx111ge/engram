/* ============================================
   engram - Chat Panel
   Floating agent-style conversational interface
   with knowledge graph grounding and active learning
   ============================================ */
...
// --- Initialize Chat (called on first panel open) ---
async function initChatIfNeeded() {
  ...
  if (statusEl) {
    if (llmConfigured) {
      ...
      statusEl.title = 'LLM Connected';
    } else {
      statusEl.innerHTML = '<i class="fa-solid fa-triangle-exclamation" style="color:var(--confidence-mid)"></i>';
      statusEl.title = 'LLM Not Configured -- set up in System settings';
    }
  }
    ...
    if (!llmConfigured) {
      messagesEl.innerHTML = `
        <div style="text-align:center;padding:1.5rem;color:var(--text-muted)">
          ...
          <p style="font-size:0.9rem;margin-bottom:0.5rem">Configure an LLM to start chatting</p>
          <p style="font-size:0.8rem">Go to <a href="#/system" onclick="document.getElementById('chat-panel').classList.remove('open');document.getElementById('chat-panel').classList.add('closed')">System</a> to set up your language model.</p>
        </div>`;
    } else {
      showWelcomeMessage();
    }
...
function showWelcomeMessage() {
  ...
  messagesEl.innerHTML = `
    <div style="text-align:center;padding:1.5rem;color:var(--text-muted)">
      ...
      <p style="font-size:0.9rem;margin-bottom:1rem">Ask me anything about your knowledge base</p>
...
function renderMessage(role, content, tools) {
  ...
  msgDiv.innerHTML = `
    ${avatar}
    <div style="max-width:85%;min-width:150px;padding:0.75rem 1rem;border-radius:var(--radius-sm);font-size:0.9rem;line-height:1.5;${bubbleAlign}">
