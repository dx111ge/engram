...
pub fn router_with_frontend(state: AppState, frontend_dir: Option<&str>) -> Router {
    ...
    let mut app = app
        ...
        .route("/chat/briefing", post(handlers::chat_reporting::briefing))
        .route("/chat/export_subgraph", post(handlers::chat_reporting::export_subgraph))
        .route("/chat/entity_timeline", post(handlers::chat_temporal::entity_timeline))
        .route("/chat/fact_provenance", post(handlers::chat_temporal::fact_provenance))
        .route("/chat/network_analysis", post(handlers::chat_investigation::network_analysis))
        .route("/chat/entity_360", post(handlers::chat_investigation::entity_360))
        .route("/chat/entity_gaps", post(handlers::chat_investigation::entity_gaps))
        .route("/chat/dossier", post(handlers::chat_reporting::dossier))
        .route("/chat/topic_map", post(handlers::chat_reporting::topic_map))
        .route("/chat/graph_stats", post(handlers::chat_reporting::graph_stats))
        .route("/chat/contradictions", post(handlers::chat_temporal::contradictions))
        .route("/chat/situation_at", post(handlers::chat_temporal::situation_at))
